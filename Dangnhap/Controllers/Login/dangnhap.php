  ...
  case 'dang_ky':
   if (isset($_POST['Dangky'])) {
    ...
    if ($txtPassword == $txtCfPassword) {
     if (Dangnhap::ADD($txtUsername,$txtPassword, $txtEmail,$txtAddress,$txtPhonenb,$txtRole)) {
  ...
  break;case 'quen_mk':
   ...
   case 'reset_password':
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
     ...
     $txtusername = $_POST['txtUsername'];
      ...
      if (Dangnhap::Edit($txtPassword, $txtEmail, $txtPhone,$txtAddress, $username)) {
       $_SESSION['username'] = $txtUsername;
       header('location:index.php?controllers=quanly&action=Admin');
      }
      else
   ...
   case 'reset_password':
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
     ...
     // Cập nhật mật khẩu trong database
     if (Dangnhap::ResetPassword($email, $new_password)) {echo "<p style='color:green;'>Mật khẩu đã được cập nhật thành công!</p>";
	 ...
	 case 'Admin':
        if (isset($_POST['login'])) {
         ...
         if (!empty($list_user)) {
          ...
          $role = $user['vaiTro']; // Lấy giá trị vai trò từ kết quả database
